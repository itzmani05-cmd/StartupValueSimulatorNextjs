-- Comprehensive Database Update Script (Safe Version)
...
-- Add constraints for data integrity (with error handling)
DO $$
BEGIN
    -- Add legal structure constraint if it doesn't exist
    IF NOT EXISTS (
        SELECT 1 FROM pg_constraint 
        WHERE conname = 'check_legal_structure' 
    ...
    -- Add total shares constraint if it doesn't exist
    IF NOT EXISTS (
        SELECT 1 FROM pg_constraint 
        WHERE conname = 'check_total_shares' 
        AND conrelid = 'companies'::regclass
    ) THEN
        ALTER TABLE companies 
    ...
    -- Add ESOP pool constraint if it doesn't exist
    IF NOT EXISTS (
        SELECT 1 FROM pg_constraint 
        WHERE conname = 'check_esop_pool' 
        AND conrelid = 'companies'::regclass
    ) THEN
        ALTER TABLE companies 
